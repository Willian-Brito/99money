...
type
  TFrm_Principal = class(TForm)
    ...
    ToolBar10: TToolBar;
    Label4: TLabel;
    btn_sobre_voltar: TSpeedButton;
    Rectangle9: TRectangle;
    ActSobre: TChangeTabAction;
    Rectangle10: TRectangle;
    Rectangle11: TRectangle;
    Image4: TImage;
    lbl_control_Finan: TLabel;
    lbl_desenv: TLabel;
    rect_controleFinan: TRectangle;
    rect_desenv: TRectangle;
    lbl_texto: TLabel;
    rect_texto: TRectangle;
    rect_Versao: TRectangle;
    lbl_versao: TLabel;
    img_despesa: TImage;
    img_receita: TImage;
    ...
    procedure item_menu_painelClick(Sender: TObject);
    procedure btn_sobre_voltarClick(Sender: TObject);
    procedure item_menu_sobreClick(Sender: TObject);
...
begin
        try
                ...
                frm_principal.lbl_saldo.Text :=  simb_moeda + FormatFloat('#,##0.00', saldo);

                //if saldo < 0 then
                //    frm_principal.lbl_saldo.Text := '- ' + frm_principal.lbl_saldo.Text;
...
begin
    ...
    begin
       ...
       //dm.qry_categoria.FieldByName('DESCRICAO').Value := edt_categoria.Text;
...
procedure TFrm_Principal.item_menu_painelClick(Sender: TObject);
begin
    // Atualiza lista de categorias...
    dm.qry_categoria.Active := false;
    dm.qry_categoria.Active := true;
...
procedure TFrm_Principal.item_menu_sobreClick(Sender: TObject);
begin

        // Esconde menu...
    MultiView.HideMaster;

    Frm_Principal.lbl_texto.Text := 'Já pensou conseguir economizar todo mês uma certa quantia de seu orçamento'+#13+
                 ' para no final do ano realizar o tão esperado sonho de viajar para o Caribe, por exemplo?'+#13+
                 ' Sabemos que você tem total capacidade de conseguir realizar este objetivo, muitas vezes'+#13+
                 ' só precisa de uma ajudinha da tecnologia, com esse aplicativo você armazena em'+#13+
                 ' categorias todas as suas despesas e receitas, de suas transações diárias, Além de planejar'+#13+
                 ' melhor para atingir o que tanto deseja, como aquela viagem que você adia há anos.';


    // Muda para aba Categoria...
    ActSobre.ExecuteTarget(sender);
...
procedure TFrm_Principal.ListCategoriasItemClick(const Sender: TObject;
  const AItem: TListViewItem);
begin
    // Preenche campo descricao...
    edt_categoria.Text := dm.qry_categoria.FieldByName('DESCRICAO').AsString;
...
procedure TFrm_Principal.btn_sobre_voltarClick(Sender: TObject);
begin
    ActMain.ExecuteTarget(sender);
end;
